...
class Buku extends BaseController
{
    ...
    public function update_data_buku()
    {
        ...
        $cover = $this->request->getFile('cover_buku');
        $namacover = ($cover && $cover->isValid() && !$cover->hasMoved())
            ? (function() use ($cover) { $n = $cover->getRandomName(); $cover->move(FCPATH.'Assets/uploads/cover/', $n); return $n; })()
            : $coverLama;

        $ebook = $this->request->getFile('e_book');
        $namaebook = ($ebook && $ebook->isValid() && !$ebook->hasMoved())
            ? (function() use ($ebook) { $n = $ebook->getRandomName(); $ebook->move(FCPATH.'Assets/uploads/ebook/', $n); return $n; })()
            : $ebookLama;
